...
#if !defined(_GLUCODUINO_H_)
#define _GLUCODUINO_H_
#include <Usb.h>

#define GLUCO_MAX_ENDPOINTS 5 //endpoint 0, bulk_IN(MIDI), bulk_OUT(MIDI), bulk_IN(VSP), bulk_OUT(VSP)
#define USB_SUBCLASS_MIDISTREAMING 3
#define DESC_BUFF_SIZE        256
#define GLUCO_EVENT_PACKET_SIZE 64
class GLUCODUINO;

class GLUCODUINO : public USBDeviceConfig
{
protected:
  ...
  EpInfo  epInfo[GLUCO_MAX_ENDPOINTS];
  /* MIDI Event packet buffer */
  uint8_t recvBuf[GLUCO_EVENT_PACKET_SIZE];
  uint8_t readPtr;
...
public:
  ...
  uint8_t SendData(uint8_t *dataptr, byte nBytes);
